...
def main(args):
    ...
    # Initialize correct early stoppers for each task
    ae_early_stopper = TrainingLossEarlyStopper(patience=cfg.EARLY_STOPPING_PATIENCE, rel_delta_threshold=1e-6)
    ...
    NUM_FEATURES_PREPROCESSED, _, original_feature_names = \
    ...
    if args.classifier_mode == "binary":
        num_classes_for_cnn, target_names_for_report = 1, ['Normal (Class 0)', 'Abnormal (Class 1)']
        ...
        num_classes_for_cnn, target_names_for_report = cfg.NUM_ORIGINAL_CLASSES, [cfg.NSL_KDD_CLASS_NAMES_INT_TO_STR[i] for i in range(cfg.NUM_ORIGINAL_CLASSES)]
        y_train_for_cnn_tensor, y_test_for_cnn_tensor = y_train_final_labels_tensor.squeeze().long(), y_test_original_labels_tensor.squeeze().long()
    ...
    if 'ae' in args.model:
        ...
        if X_train_features_ae is None:
            ...
            ae_train_loader_for_fe = DataLoader(
                ae_train_dataset_for_fe, batch_size=cfg.AE_BATCH_SIZE, shuffle=True, 
                num_workers=4, pin_memory=True
            )
            
            ae_early_stopper.reset()
            feature_extraction_ae, ae_history = train_autoencoder(feature_extraction_ae, ae_train_loader_for_fe, cfg.DEFAULT_EPOCHS, cfg.AE_LEARNING_RATE, cfg.DEVICE, early_stopper=ae_early_stopper)
            step_timings['Feature Extraction AE Training'] = format_time(time.time() - start_time_s3train_ae)

            ae_graph_filename = f"training_graph_AE_{'gan' if args.use_gan else 'orig'}_{'outlier' if args.use_outlier_removal else 'no_outlier'}.png"
            save_training_graphs(ae_history, "Feature Extraction Autoencoder", output_path=ae_graph_filename)
            
            print("\n--- Starting: AE Feature Extraction ---")
            start_time_s3extract = time.time()
            ae_train_extract_loader = DataLoader(
                NSLKDDDataset(X_train_final_tensor), batch_size=cfg.AE_BATCH_SIZE, shuffle=False, 
                num_workers=4, pin_memory=True
            )
            ae_test_extract_loader = DataLoader(
                NSLKDDDataset(X_test_original_tensor), batch_size=cfg.AE_BATCH_SIZE, shuffle=False, 
                num_workers=4, pin_memory=True
            )
    ...
    X_for_split, y_for_split = X_train_for_cnn.cpu().numpy(), y_train_for_cnn_tensor.cpu().numpy()
    
    try:
        y_stratify = y_for_split if y_for_split.ndim == 1 else y_for_split.ravel()
        X_train_split, X_val_split, y_train_split, y_val_split = train_test_split(X_for_split, y_for_split, test_size=0.2, random_state=42, stratify=y_stratify)
    except ValueError:
        print("  Warning: Cannot stratify split, likely due to a class having too few samples. Splitting without stratification.")
        X_train_split, X_val_split, y_train_split, y_val_split = train_test_split(X_for_split, y_for_split, test_size=0.2, random_state=42)

    train_dataset = NSLKDDDataset(
        torch.from_numpy(X_train_split).clone(), 
        torch.from_numpy(y_train_split).clone()
    )
    val_dataset = NSLKDDDataset(
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=cfg.CNN_BATCH_SIZE, shuffle=True, 
        num_workers=4, pin_memory=True
    )
    val_loader = DataLoader(
        val_dataset, batch_size=cfg.CNN_BATCH_SIZE, shuffle=False, 
        num_workers=4, pin_memory=True
    )
    test_loader = DataLoader(
        test_dataset, batch_size=cfg.CNN_BATCH_SIZE, shuffle=False, 
        num_workers=4, pin_memory=True
    )
    
    print(f"Data ready for Classifier. Train samples: {len(train_dataset)}, Validation samples: {len(val_dataset)}, Test samples: {len(test_dataset)}")
    
    print(f"\n--- Instantiating Classifier Model: {args.model.upper()} ---")
    classifier_model = None
    model_type = args.model.replace('ae', '')
    ...
    if model_type == 'cnn':
        classifier_model = CNNClassifier(
            ...
            cnn_pool_stride=cfg.CNN_POOL_STRIDE,
        ...
        classifier_model = DNNClassifier(
            ...
            hidden_layer_2=cfg.DNN_HIDDEN_LAYER_2,
            num_classes=num_classes_for_cnn
        )
    ...
    if args.classifier_mode == 'multiclass':
        ...
        y_train_for_weights = y_train_for_cnn_tensor.cpu().numpy()
        weights = compute_class_weight(class_weight='balanced', classes=np.unique(y_train_for_weights), y=y_train_for_weights)
        class_weights = torch.tensor(weights, dtype=torch.float).to(cfg.DEVICE)
        print(f"Calculated weights for classes {np.unique(y_train_for_weights)}: {weights}")
    
    trained_classifier_model, classifier_history = train_classifier(
        classifier_model, train_loader, val_loader, cfg.CNN_EPOCHS, 
        cfg.CNN_LEARNING_RATE, cfg.DEVICE, 
        num_classes=num_classes_for_cnn, early_stopper=classifier_early_stopper,
        target_names_report=target_names_for_report, class_weights=class_weights
    )
    step_timings['Classifier Training & Validation'] = format_time(time.time() - start_time_s4)

    base_filename = f"{cfg.TABLE_OUTPUT_FILENAME_PREFIX}_{args.model}_{'gan' if args.use_gan else 'orig'}_{'outlier' if args.use_outlier_removal else 'no_outlier'}_{args.classifier_mode}"
    classifier_graph_filename = f"{base_filename}_training_graph.png"
    save_training_graphs(classifier_history, f"{args.model.upper()} Classifier", output_path=classifier_graph_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Select the model pipeline to use (REQUIRED):\n"
             ...
             "  'dnn', 'lstm', etc. are placeholders for future models."
    )
