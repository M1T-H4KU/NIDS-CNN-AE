...
def main(args):
    ...
    print(f"Running with arguments: GAN Augmentation={'Enabled' if args.use_gan else 'Disabled'}, Classifier Mode='{args.classifier_mode}', Model='{args.model}'")
    step_timings = {}
    ...
    print(f"Original training data: X={X_train_original_tensor.shape}, y_multi={y_train_original_multiclass_tensor.shape}")
    print(f"Original test data: X={X_test_original_tensor.shape}, y_multi={y_test_original_multiclass_tensor.shape}")
    ...
    if args.use_gan:
        ...
        print(f"Found unique multi-class labels for BEGAN training: {unique_classes}")
            ...
            if num_to_generate_for_class > 0 and trained_g_class is not None:
                 ...
                 if synthetic_class_features_cpu.size(0) > 0:
                     synthetic_class_labels_cpu = torch.full(
                        ...
                        float(class_label), dtype=torch.long
                    )
        ...
        step_timings['Class-wise BEGAN Training & Augmentation'] = format_time(duration_s2)
    ...
    num_classes_for_cnn, y_train_for_cnn_tensor, y_test_for_cnn_tensor, target_names_for_report = 0, None, None, None
    ...
    # Initialize early stoppers for AE and Classifier training
    ...
    classifier_early_stopper = TrainingLossEarlyStopper(patience=cfg.EARLY_STOPPING_PATIENCE, rel_delta_threshold=cfg.EARLY_STOPPING_REL_DELTA)
    ...
    if X_train_features_ae is None:
        ...
        ae_train_dataset_for_fe = NSLKDDDataset(X_train_final_tensor.to(cfg.DEVICE))
    ...
    X_train_features_ae, X_test_features_ae = X_train_features_ae.to(cfg.DEVICE), X_test_features_ae.to(cfg.DEVICE)
    ...
    print(f"\n--- Starting: {args.model.upper()} Classifier Training & Validation (Mode: {args.classifier_mode}, Classes: {num_classes_for_cnn}) ---")
    ...
    print(f"--- Finished Classifier Training & Validation in {format_time(time.time() - start_time_s4)} ---")
    ...
    print(f"--- Finished: Final CNN Evaluation in {format_time(time.time() - start_time_s5)} ---")
    ...
    table_output_file = f"{cfg.TABLE_OUTPUT_FILENAME_PREFIX}_{args.model}_{'gan' if args.use_gan else 'orig'}_{args.classifier_mode}.png"
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Set the classifier mode (REQUIRED):\n"
             "  'binary':   Binary classification (Normal vs. Attack).\n"
             "  'multiclass': Multi-class classification (Normal, DoS, Probe, etc.)."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=['cnn', 'dnn', 'lstm'],
        help="Select the classifier model to use (REQUIRED):\n"
             "  'cnn':  Convolutional Neural Network (currently implemented).\n"
             "  'dnn':  Deep Neural Network (to be implemented).\n"
             "  'lstm': Long Short-Term Memory Network (to be implemented)."
    )
